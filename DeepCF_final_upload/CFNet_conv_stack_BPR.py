...
#             nn.ReLU()
        ...
        self.cnn_final = nn.Sequential(
            # batch_size * 2 * 32 * 32
            nn.Conv2d(2, self.channel_size_final, self.kernel_size_final, stride=self.strides_final),
            nn.ReLU(),
            # batch_size * 32 * 16 * 16
    ...
    def forward(self, user, item):
        ...
        stacked_map = torch.stack([interaction_map_dmf, interaction_map_mlp], dim=1).squeeze()
...
if __name__ == "__main__":
    ...
    for epoch in range(args.epochs):
        ...
        print(f"Epoch {epoch+1}: BPR_Loss={losses.avg:.4f} [{time.time()-t1:.1f}s]")
